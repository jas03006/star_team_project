...
public class UserData
{
    ...
    public List<ArrayList> StageInfo_List = new List<ArrayList>();         //스테이지 별 정보
    public List<ArrayList> Housing_List = new List<ArrayList>();           //하우징 정보
    public List<ArrayList> QuestInfo_List = new List<ArrayList>();         //퀘스트 별 클리어 여부
    public List<ArrayList> Achievements_List = new List<ArrayList>();      //업적 별 클리어 여부 
    ...
    public void GameDataGet()
    {
        ...
        if (bro.IsSuccess())
        {
            ...
            else
            {
                ...
                foreach(string itemKey in gameDataJson[0]["Friend_UUID_List"])  //친구정보
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Friend_UUID_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Character_ID_List"]) //보유 캐릭터 리스트
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Character_ID_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Char_Item_ID_List"])   //캐릭터 아이템 리스트
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Char_Item_ID_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Adjective_ID_List"])  //형용사 칭호 리스트
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Adjective_ID_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Noun_ID_List"])  //명사 칭호 리스트
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Noun_ID_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["House_Item_ID_List"])  //하우징 아이템 리스트
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["House_Item_ID_List"][itemKey].ToString()));
                }
                ...
                //}
                foreach (string itemKey in gameDataJson[0]["StageInfo_List"]) //스테이지 별 정보
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["StageInfo_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Housing_List"])//하우징 정보
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Housing_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["QuestInfo_List"])//퀘스트 별 클리어 여부
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["QuestInfo_List"][itemKey].ToString()));
                }
                foreach (string itemKey in gameDataJson[0]["Achievements_List"])//업적 별 클리어 여부 
                {
                    userData.inventory.Add(itemKey, int.Parse(gameDataJson[0]["Achievements_List"][itemKey].ToString()));  
                }
                //예제 코드
